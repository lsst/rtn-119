...
The DP1 butler repository at FrDF is a community repository whose purpose is to make DP1 data (produced at USDF) available to the IN2P3 science community. Its content can be accessed directly from the FrDF interactive, computing and notebook platforms for analysis. Access can be configured in the same way as the "main" repository, as documented `here <https://doc.lsst.eu/tutorial/main_butler.html>`__.
...
We use the same ``skymap lsst_cells_v1.skymap.config`` as the one mentioned in `RTN-103 <https://rtn-103.lsst.io>`__.
